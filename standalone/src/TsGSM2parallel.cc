...
TsGSM2::TsGSM2(double yF, double yD, double Rd, double Rc, double kinA, double kinB, double kinR, string GSM2_ion, double GSM2_LET,
	...
	:GSM2Model_yF(yF), GSM2Model_yD(yD), GSM2Model_rd(Rd), GSM2Model_rc(Rc), GSM2_a(kinA), GSM2_b(kinB), GSM2_r(kinR), GSM2_ion(GSM2_ion), GSM2_LET(GSM2_LET),
	fyVector(yVector), fyVector_Particle(yVector_Particle), fyVector_Nucleus(yVector_Nucleus), fyVector_Particle_Nucleus(yVector_Particle_Nucleus), fGetStatisticInfo(GetStatisticInfo), fSpectrumUpdateTimes(SpectrumUpdateTimes)
{
	...
	// New Kappa formulation (LET vs yF map from PIDE)	
	//double nDBS = CalculateKappaFromLET(GSM2_ion, GSM2_LET);
	// R = 8 um
	//double nDBS = CalculateKappaFromLET(GSM2_ion, 0.88 + 0.86*yF);
	// R = 0.5 um
	
	//double nDBS = CalculateKappaFromLET(GSM2_ion, pow(10,0.01242109 + 0.9922239*log10(yF)));
	double nDBS = CalculateKappaFromLET(GSM2_ion, pow(10,0.01242109 + 0.9922239*log10(yD)));
...
{		
	// Conversion factor z -> y
	double z_to_y_factor = (4*GSM2Model_rd*GSM2Model_rd)/0.204;
	// Conversion Kappa
	double kappa_conversion = pow(GSM2Model_rd/GSM2Model_rc,3);
